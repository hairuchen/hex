...
public class RabbitmqProducer implements MessageProducer, MessageConsumer {
    ...
    public void consumeParseMessage(String fileId) {
        try {
            ...
            // 2. 从 MinIO 下载文件
            String objectPath = fileEntity.getMinioObjectPath(); // 从数据库获取存储路径
...
// ===================== 3. 存入 Neo4j（知识片段 + 顺序关系） =====================
        ...
        for (String chunk : chunks) {
            ...
            // 构建知识片段节点
            KnowledgeChunkNode node = new KnowledgeChunkNode();
            node.setChunkId(UUID.randomUUID().toString());
            node.setFileId(fileId);
            node.setContent(chunk.trim());
            node.setType("TEXT");
            node.setIndex(index++);
            node.setTokenCount(chunk.length());
            node.setCreateTime(LocalDateTime.now());
            node.setSource("UPLOAD");
            node.setStatus("NORMAL");

            // 保存到 Neo4j
            knowledgeService.createChunk(node);

            // 构建 NEXT_CHUNK 顺序关系
            if (previousChunkId != null) {
                knowledgeService.createNextChunkRelation(previousChunkId, node.getChunkId());
            }

            previousChunkId = node.getChunkId();
        }
