...
# 项目详细介绍
该知识库系统对比 RagFlow、Dify 等主流产品的核心特点：
<b>

“知识图谱 + RAG” 深度融合</b>

结构化知识建模： 实现 “文本分片→实体抽取→关系构建→图谱入库” 全链路，支持 NEXT_CHUNK 顺序关系、Chunk→Entity 关联关系、实体间语义关系的显式存储；

多路召回的深度定制：实现 “文本匹配 + 向量检索 + 图谱关联” 的混合召回，可通过配置权重（RetrieveWeightProperties）调优检索策略，比主流产品的 “固定混合检索” 更灵活；

面向 “知识沉淀” 而非仅 “检索”：主流产品侧重 “检索问答”，本系统额外关注 “知识结构化”（实体 / 关系的持久化、图谱遍历），适合需要构建行业知识图谱的场景（如医疗、金融、政务），也有助于企业沉淀属于自己的垂直领域知识库。
